...
class GenericBuilder {
	constructor(config, environment) {
		...
		this.baseSourcePath = this.environment.martelo.buildConfig.baseSourcePath;
	...
	async run() {
		...
		log(
			`Building ${log.formatEnvironment(this.environment.key)} ${log.formatType(this.constructor.builderType)}`,
			"INFO"
		);
		...
		log.timeEnd(
			`${log.formatEnvironment(this.environment.key)} ${log.formatType(this.constructor.builderType)}`,
			startTime,
			"INFO"
		);
	}
