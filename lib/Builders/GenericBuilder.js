...
class GenericBuilder {
	...
	reloadBrowserSync(browserSync) {
		if (browserSync === void 0) {
			return;
		}

		browserSync.reload(this.filePaths);

		if (this.filePaths.length === 1) {
			Logger.log(`[[a:sync]] ${this.filePaths[0]} has been reloaded`);
		}
		else if (this.filePaths.length > 1) {
			Logger.log(`[[a:sync]] ${this.filePaths.length} files have been reloaded`);

			Logger.log([`[[a:sync]] Reloaded files:`].concat(this.filePaths));
		}
	}
	...
	updateFileList(buildFiles) {
		for (const buildFile of buildFiles) {
			if (Array.isArray(buildFile)) {
				...
				this.filePaths.push((buildFile.revisionedDestination || buildFile.destination));
